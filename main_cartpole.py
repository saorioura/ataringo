...
if __name__ == "__main__":
    ...
    parser.add_argument("--tau", default=0.5, type=float)  # Target network update rate
    ...
    # Initialize policy
    if args.policy == "DQN":
        policy = DQN(state_dim, action_dim, max_action)
    else:
    ...
    while total_timesteps < args.max_timesteps:
        if total_timesteps > args.start_timesteps:
            if args.policy == "DQN":
                td_errors = policy.train(replay_buffer, batch_size=args.batch_size)
                tf.summary.scalar(name="td_errors", data=td_errors, step=total_timesteps)
            else:
