...
import utils
from ddpg_cnn_continuous import DDPG
import cv2
...
class atari_wrapper(gym.Wrapper):
    ...
    def step(self, ac):
        ...
        ctrl = ac #np.array([ac[0], 0.5 * ac[1] + 0.5, 0.5 * ac[2] + 0.5])
    ...
    while total_timesteps < args.max_timesteps:
        if total_timesteps > args.start_timesteps:
            if args.policy == "SAC":
                total_actor_loss, total_v_loss, total_q_loss = policy.train(replay_buffer,
                                                                            ...
                                                                            tau=args.tau)
                tf.summary.scalar(name="V_Loss", data=total_v_loss, step=total_timesteps)
                tf.summary.scalar(name="Q_Loss", data=total_q_loss, step=total_timesteps)
        ...
        if done:
            ...
            if timesteps_since_eval >= args.eval_freq:
                ...
                tf.summary.scalar(name="online_return", data=cur_evaluate, step=total_timesteps)
                writer.flush()
        ...
        reward, action = reward, action.astype(np.float32)
