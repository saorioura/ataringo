...
class DQN(tf.keras.Model):
    def __init__(self, state_dim, action_dim, max_action, name="DQN", epsilon=0.1,):
        super().__init__()
        ...
        self.epsilon_min = 1e-5
        self.epsilon_decay = 1e9
    ...
    def train(self, replay_buffer, batch_size=64, discount=0.99):
        ...
        self.epsilon = max(self.epsilon_min, self.epsilon - self.epsilon_decay_rate * self.n_update)
    ...
    def _train_body(self, state, next_state, action, reward, not_done, discount):
        with tf.device("/cpu:0"):
            ...
            with tf.GradientTape() as tape:
                ...
                q_func_loss = tf.reduce_mean(huber_loss(td_error, delta=1000.))
