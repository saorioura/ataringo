...
class Actor(tf.keras.Model):
    def __init__(self, state_dim, action_dim, max_action, name="Actor"):
        ...
        self.l1 = layers.Conv2D(32, 3, strides=2, padding="same", name="L1")
        self.l2 = layers.Conv2D(32, 3, strides=2, padding="same", name="L2")
        self.l3 = layers.Flatten()
        self.l4 = layers.Dense(64, kernel_initializer=PytorchInitializer, name="L4")
    ...
    def call(self, inputs):
        with tf.device("/cpu:0"):
            ...
            # action = tf.keras.activations.softmax(features, axis=2)
...
class Critic(tf.keras.Model):
    def __init__(self, state_dim, action_dim, wd=1e-2, name="Critic"):
        ...
        self.l2 = layers.Conv2D(32, 3, strides=2, padding="same", name="L2")
        self.l3 = layers.Flatten()
        self.l4 = layers.Dense(64, kernel_initializer=PytorchInitializer, kernel_regularizer=regularizers.l2(wd),
                               bias_regularizer=regularizers.l2(wd), name="L4")
        self.l5 = layers.Dense(1, kernel_initializer=PytorchInitializer, kernel_regularizer=regularizers.l2(wd),
                               bias_regularizer=regularizers.l2(wd), name="L5")
    ...
    def call(self, inputs):
        with tf.device("/cpu:0"):
            ...
            features = tf.nn.relu(self.l1(x))
            features = tf.nn.relu(self.l2(features))
            x = self.l3(features)

            # x = tf.nn.relu(self.l1(x))
            inner_feat = tf.concat([x, u], axis=1)
            x = tf.nn.relu(self.l4(inner_feat))
            x = self.l5(x)
        return x
...
class DDPG(tf.keras.Model):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.actor_optimizer = tf.keras.optimizers.Adam(learning_rate=1e-2)
        ...
        self.critic_optimizer = tf.keras.optimizers.Adam(learning_rate=2e-2)
